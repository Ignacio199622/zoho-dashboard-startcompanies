...
const CAMPOS = 'id,Full_Name,Email,Phone,Mobile,Lead_Status,Description,Owner,Retargeting,Nombre_retargeting';
  ...
  try {
    ...
    for (const m of nuevas) {
      ...
      try {
        ...
        const caso = await crear({ llamada, analisis, coach, lead, ventana, telefono, mail });
        const r = await publicar(caso);
        await actualizar(caso.id, { slack: { canal: r.canal, ts: r.ts } });

        yaPublicadas.add(clave);
        publicadasAhora++;
        await dormir(600);
      } catch (e) {
    ...
    // La lista se poda para que el blob no crezca para siempre.
